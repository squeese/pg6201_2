...
export default () => {
  ...
  const { state:{ camera, light }} = useContext(Context);
  ...
  useEffect(function InitializeThreeJSApplication() {
    ...
    app.camera._rotTar = new T.Quaternion();
    app.camera._rotCur = new T.Quaternion();
    app.camera._position = new T.Vector3(1, -1.5, 3.5);
    app.camera._origin = new T.Vector3(0, -1.75, 0);
    // THREE.WebGLRenderer
    ...
    // Mesh: TmpBox
    app.tmpbox = new T.Mesh(
      new T.BoxGeometry(2, 2, 2),
      new T.MeshBasicMaterial({ wireframe: true }));
  ...
  useEffect(function OptionsLightChange() {
    app.tmpbox.matrixAutoUpdate = false;
    app.tmpbox.matrix.makeRotationFromEuler(euler.set(...light.rotation, 'YXZ'));
    app.tmpbox.matrix.multiply(matrix.makeScale(...light.scale));
    app.tmpbox.matrix.multiply(matrix.makeShear(...light.shear));
    app.tmpbox.matrix.multiply(matrix.makeTranslation(...light.translation));
    app.particles.material.uniforms.uTest.value.getInverse(app.tmpbox.matrix);
  }, [app, light, matrix, euler]);

  useEffect(function MouseEventsForCamera() {
    return utils.mouseMoveHandler(canvas.current, delta => {
      app.camera._rotTar.multiply(delta);
    });
  }, [app, canvas]);
  ...
  useEffect(function StartStopRenderLoop() {
    let request = requestAnimationFrame(function frame() {
      ...
      app.camera._rotCur.slerp(app.camera._rotTar, 0.1);
      app.camera.position.copy(app.camera._position);
      app.camera.position.applyQuaternion(app.camera._rotCur);
      app.camera.lookAt(app.camera._origin);
      // ??
      // render stuff
...
const particles = gl => {
  const material = new T.ShaderMaterial({
    ...
    uniforms: { uTest: { value: new T.Matrix4() }},
    onBeforeLink: program => gl.transformFeedbackVaryings(program, ["vPosition", "vVelocity", "vDuration"], gl.SEPARATE_ATTRIBS),
    transparent: true,
  });
